...
class PathBezier3(object):
	...
	def render(self, params):
		...
		return PathBezier4(self.start, (c1x,c1y), (c2x,c2y), self.end,self.color).render()
	def reverse(self):
		...
		while tokens:
			...
			if ucmd == 'M':
				...
				if (sp_start[0] - cur[0]) > 0.0000001 or (sp_start[1] - cur[1]) > 0.0000001:
					subpath.add(PathLine(cur, sp_start,True,sel),self.color)
				cur = curcpc = curcpq = sp_start
...
class SVGReader(xml.sax.handler.ContentHandler):
	...
	def endElement(self, name):
		if name == 'g':
			#self.popmatrix() Removing this is a super temporary fix that works for adobe transforms
...
def write_ild(fout, params, rframe, path, center=True):
	global number_of_frames, current_frame
	...
	for i in range(frames):
		hdr = struct.pack(">4s3xB8s8sHHHBx", b"ILDA", 1, b"svg2ilda", b"", samples, current_frame, number_of_frames, 0)
		fout.write(hdr)
		fout.write(dout)
...
if __name__ == "__main__":
	...
	for path in onlyfiles:
		...
		if verbose:
			print("Done")
		write_ild(fout, params, rframe, sys.argv[2], center)
